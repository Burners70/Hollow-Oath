# Tester log

Tracks potential playtesters for the TestFlight rounds referenced in
`docs/APP_STORE_ROADMAP.md` (Bundle O6 — internal build, then a 5–10 player
external round) and `docs/RELEASE_READINESS_REVIEW.md`.

Status values:
- *(blank)* — identified as a candidate, not yet contacted
- `Approached` — invited, awaiting response
- `Agreed` — confirmed, will test

| Name | Email | Status |
|------|-------|--------|
| Liam McKenzie | McKenzie_92@msn.com | Agreed |
| Joe Tipping | joetipping@hotmail.com | Agreed |
| Terry Grey | TBC | Approached |
| Phil Burnett | philip167@msn.com | Agreed |
| Rachel Burnett | rpearson@live.co.uk | Agreed |
| Richard Lowther | TBC | Agreed |
| Stephen Garside | steve.garside@accenture.com | Agreed |
| James Burnett | TBC | Approached |

...
| `js/acttwo-update.js`   | ~1445 | Act Two's loop (`updateActTwo`, a no-op unless the level carries racks): the tether (`updateTow`, `towContact`, `cradleRack`, `seatPayload`, `partSling`), the trunk cut (`closeTrunk`, `actTwoShotHit`), the draining reserve and its bite on the 41s beat (`updateReserves`), loose-payload physics, the inverted transfusion (`updateGive`), THE WELL (`updateWellDock`, `deliverRack`), `hullShieldBounce`, `respawnInChamber`; P·systems' **score ladder** — `a2Award`/`a2Charge` (nothing else may touch `score` in Act Two) and `checkChamberClear`; and §8.1's **deception tell** (`updateLies`/`touchLie`/`probeLies`) |
...
| `js/debug.js`    | ~690  | The `window.__doids` test/debug handle — `get()`, `go(n)`, `launch()`, `warpLift()`, `give()`, `loadChamber()`, `reset()` and the property-exposing helpers the smoke suite asserts on. Every test drives this |
...
four, so it is tested against a purpose-built chamber via `__doids.enterChamber`.

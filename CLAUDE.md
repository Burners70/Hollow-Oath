...
| `js/update.js`   | ~3220 | The 41-second Static clock; landing rules + extraction/MERCY; `update(dt)` dispatch; rank system; per-screen + gameplay updates (`updatePlay`, oids, enemies, sabotage, scan/reveal, docking, blackbox, transfusion, lifts, counterfeit MERCY, epilogue) |
| `js/render.js`   | ~5160 | `render()` dispatch + `drawGlow`/perf helpers; world render (terrain, darkness, ship, drone, oids, scenery); figures; counterfeit MERCY; HUD/health/ECG; all screens (title, codex, intro, brief, clear, pause, settings, game over, win); the `window.__doids` debug handle |
...
| File | Lines | Covers |
|------|-------|--------|
| `js/acttwo-data.js`   | ~2145 | **Landed (P·design):** the rack's token/state layer — `RACK_STATES`, `RACK_PULSE_PERIOD`, the beat envelope, `rackColor`/`rackBrightness`, `PLANT_ZONES`/`plantPal` — same data/no-drawing split as `PAL()`/`TOK` in `js/world.js`. **Landed (P·terrain):** the chamber authoring grammar (`room`/`rock` parts with per-boundary roughness) and its compiler — `compileChamber`, `spanUnion`/`spanSubtract`, `chamberNoise`, `spanCountAt` — plus `SLICE_CHAMBER` (the one chamber, proving the format) and `genChamber` (compiles a chamber to a level-shaped object, **terrain only**). A part may declare a **view** (`drawn`/`solid`), which is how §8's false floors and painted rock are held: `genChamber` compiles `spans` (collision truth) and `spansDrawn` (what you see) from one definition, identical unless a deception is declared. Chambers also carry **lights** and **ornaments**, snapped to real surfaces. `RACK_SIZE`/`RACK_OCCUPANTS_DEFAULT` size a rack against what a laden ship can lift through a chamber, and `SLING_L`/`towEnvelope`/`towTierForGap` classify every gap as passable at rest, a **momentum pinch** (only with the load swung up to your level), or unladen-only. Per-boundary **materials** (`MAT_ROCK`/`MAT_MACH`, `ROCK_PAL`) carry the owner's "rock overhead, mechanical underfoot" rule, and boundary **profiles** (`boundaryProfile` — `ramp`/`arc`/`teeth`, plus `cornerInset` fillets) keep chambers off pure right angles. **Landed (P·slice):** the tether's feel dials in one block (`SLING_SHIP_W`/`SLING_DAMP`/`SLING_SAFE_V`/`SLING_DMG_K`, and `SLING_VISIBLE` as the one dial that derives `SLING_L` and with it the momentum band and both authored gaps), the reserve model (`RACK_RESERVE_MAX`/`RACK_DRAIN`/`RACK_BEAT_BITE`/`RACK_FAILING_AT`), the transfusion's (`GIVE_*`) and the well's (`WELL_*`), plus the slice chamber's authored `racks`/`conduits`/`well` and the `buildRacks`/`buildConduits` that turn them into run state. Its structural column was re-authored here: it was floor-to-ceiling, which made it a wall that sealed the only route — see the roadmap. **Landed (P·floor):** the **feature vocabulary** — the layer P·content authors against, and the thing the owner asked to cascade to the other nine chambers rather than the shape itself. `partList` flattens it; `hall`/`hallAt` carry the floor and the roof as one ruled profile between **stations** (`{x, ceil, floor}`, optionally changing material from there eastward), and `gallery`/`bore`/`shaft`/`shelf`/`bench`/`column`/`pinch`/`stalactites`/`falseFloor`/`paintedRock` hang named features on it. No new terrain capability — every helper emits the same `room`/`rock` parts — but three rules are enforced by construction: `column()` opens headroom over its capital **derived from the tow envelope** (a sealed column and a route past it are mutually exclusive), `pinch()` emits the pinning pair at a named tier rather than a number, and `hall()` interpolates between stations so a narrow *climbing* passage cannot read as a wall to `chamberRoute`. `SLICE_CHAMBER` is re-authored from it as a 23-station floor whose deck moves 540px and whose clear band runs 260–870px; every fixture takes its y from `hallAt`, so moving a station moves the furniture. `surfaceAcross` then snaps an extended object across its whole **footprint** and takes the deepest floor, so furniture sinks into the deck rather than floating off one end of it (ceilings deliberately invert — a buried lamp is no lamp). **Landed (P·ramp):** the act is a LADDER, not one room. `hallRefs(stations)` gives each chamber its own `deck`/`roof` fixture helpers (the four module consts closed over `SLICE_HALL` by name, which is the first thing in the way of a second chamber), and `ACT_TWO_CHAMBERS` is now `[BREACH_CHAMBER, WARDS_CHAMBER, SLICE_CHAMBER]` in play order with an `n` on each. **The ten-row teaching ladder — what every chamber introduces, and why size is monotonic — is the comment block above `BREACH_CHAMBER`, and it is the thing P·content authors against.** Chamber one teaches the tether and the delivery criterion and nothing else; chamber two adds the deduction and a rest-tier gap; the slice, unchanged, is chamber three. **Still to come:** chambers four to ten (P·content), Act Two's story tables and its own rank ladder (P·scions) |
| `js/acttwo-render.js` | ~1590 | **Landed (P·design):** the rack cage, conduit real-vs-fake tell, the network ripple/dip (riding the real `staticClock`), directional edge bleed, ornamentation, the sling and the well's bay/winch — gated on level fields no `genLevel` path sets yet (inert no-ops today). **Landed (P·terrain):** span terrain drawing — `buildSpanTile`/`getSpanTiles`/`drawChamberTerrain` draw a chamber's rock as the *complement* of its spans, reusing Act One's per-512px tile cache contract (`tileTouch`/`TILE_CACHE_CAP`/`invalidateTiles`), `drawMachinedPanelTicks` walks spans so an overhang's underside gets ticked too, and `drawChamberLights` lays additive light pools plus an ambient lift over the terrain (§9.2 — a plant is lit *by* something). Drawing reads `level.spansDrawn`; collision reads `level.spans`. The provisional heightmap stand-in this file shipped with is gone. **Landed (P·slice):** the pieces the loop needs to be readable — `drawConduits`/`drawIsolator` (the trunks and the breakers you land at), `drawHoldRing` (one helper for all three holds), `drawRackECG` (§7.3's reserve trace, weakening in *amplitude* and never in rate, drawn on the box because §7.5 refuses a HUD ward), `drawTowedRack`, `drawSlingTell` (§4.4's `✓`/`!`/`✕` at the tether midpoint, thresholded on the real damage threshold) and `drawGiveLine` (the transfusion, with the bead running *down* it). `drawWellDock` now draws its winch rack only while a load is actually being seated. **Landed (P·floor, later rounds):** a span with no *overlapping* continuation ends in a stroked FACE — and the face is the rock BAND between two spans, its own height, not the full height of the air beside it (drawing that was how a mezzanine's tip became a wall you could fly through). `drawLightFitting` is scaled up (per-fixture `fit`); the well's cable is thick and runs to the top of the world; **both `drawChamberLights` and `drawPlantOrnaments` cull to the view**, which is what makes a densely furnished chamber free. **The ornament set is owner-and-state**: `ornTone` maps `owner` (hers/his) to a body tone and `state` (dead/failing/live) to a PAL() accent, with `failing` stuttering on the real 41s beat; `ornBody` gives every piece a dark body, a shadowed face and a lit top edge. Kinds are `stretcherBay`/`oxyBank`/`medCrates`/`dripStand` (hers, wrecked), `readerHead`/`pumpSet`/`cableLoom` (his, installed over), and `ventGrate`/`gantry`/`conduitRun` (structure). **Landed (P·ramp):** a feed line CARRIES A CURRENT — a beat emitted per heartbeat travelling at a fixed speed (so spacing is the same on every trunk), a dashed overlay flowing at the same speed between them, and the run dimmed per stretch off `solidAt` where it passes through rock. And the terrain's edge pass, punch quads and machined band all use `matchBoundaryMutual` (js/world.js): the span-level match paired a shelf's pad with the hall deck across 16px of x and drew a glowing near-vertical line at the end of every overhang in the act. `ornBody` rules a gradient down a body lifted clear of the rock's own value range, and **the whole furniture layer is drawn inside the chamber branch, before `drawChamberLights`**, so the room's lamps light its furniture instead of leaving it a dark shape punched through a lit wall. **Still to come:** the ward's four readability channels (P·systems), Act Two screens |
| `js/acttwo-update.js` | ~1190 | **Landed (P·slice):** the whole Act Two loop, in one call from `updatePlay` (`updateActTwo`, a no-op unless the level carries racks). The tether — PENDULUM_SPEC §4.1's verlet point + rope constraint, with the payload taking the positional correction and the ship taking its 30% share as a **velocity impulse against the radial closing speed** (`err/dt` made the tug framerate-dependent and unbounded); `seatPayload` is the recovery path for cradles, warps and lift beats. The trunk cut at floor-mounted isolators (`closeTrunk`, and `actTwoShotHit` for §7.1's "shooting a feed dumps the rack"), the cradle (`cradleRack`), release/`partSling`, the reserve's continuous drain plus its bite on the real 41s beat (`updateReserves`, reading `staticBeat`), damage on the **normal** component of contact (`towContact` — it costs reserve *and* integrity, see the roadmap), loose-payload physics so a dropped rack falls (`updateLooseRacks`), the inverted transfusion on a held SHIELD (`updateGive`/`giveWanted`), THE WELL's dock-and-winch (`updateWellDock`/`deliverRack`), and `respawnInChamber` for a death in a chamber. **Landed (P·ramp):** `hullShieldBounce` — the field turns a chamber wall into a bounce off `solidNormal`, which is Act One's rule and was the one answer to a bad approach that a chamber's lateral rock never had. **Still to come:** pulse-reading's honest-versus-metronomic layer, the §8.1 deception tells, deep readers, chamber checkpointing (P·persist) |

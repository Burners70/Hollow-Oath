...
| `js/acttwo-update.js`   | ~1330 | Act Two's loop (`updateActTwo`, a no-op unless the level carries racks): the tether (`updateTow`, `towContact`, `cradleRack`, `seatPayload`, `partSling`), the trunk cut (`closeTrunk`, `actTwoShotHit`), the draining reserve and its bite on the 41s beat (`updateReserves`), loose-payload physics, the inverted transfusion (`updateGive`), THE WELL (`updateWellDock`, `deliverRack`), `hullShieldBounce`, `respawnInChamber`; and P·systems' **score ladder** — `a2Award`/`a2Charge` (nothing else may touch `score` in Act Two) and `checkChamberClear` |
| `js/update.js`   | ~3320 | The 41-second Static clock; landing rules + extraction/MERCY; `update(dt)` dispatch; rank system; per-screen + gameplay updates (`updatePlay`, oids, enemies, sabotage, scan/reveal, docking, blackbox, transfusion, lifts, counterfeit MERCY, epilogue) |
...
| `js/debug.js`    | ~655  | The `window.__doids` test/debug handle — `get()`, `go(n)`, `launch()`, `warpLift()`, `give()`, `loadChamber()`, `reset()` and the property-exposing helpers the smoke suite asserts on. Every test drives this |
...
`P·ramp` have landed, and `P·systems`' **score ladder** with them (the rest of
`P·systems` — the §8.1 tell, pulse-reading, deep readers, the well deepening,
the remaining ward channels, geology, husks, the rank names — has not).
...
`doids_run` (resume snapshot — **schema v2** since P·systems; `validRun` accepts
v1 and migrates, because an invalid save is *deleted* on the next boot and a
strict version check would wipe every shipped one), `doids_hi` (hiscore),
`doids_a2hi` (Act Two's own hiscore — P·systems rule 4), `doids_codex`, `doids_logs`,
